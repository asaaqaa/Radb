...
def telethon_session():
    ...
    # logging in
    try:
        with TelegramClient(StringSession(), API_ID, API_HASH) as Reed:
            print("Generating a string session for •Reed•")
            try:
                Reed.send_message(
                    "me",
                    f"**- هـذا هو كـود سيشـن تيليثـون الخـاص بـك ☑️**:\n\n`{Reed.session.save()}`\n\n**- لا تشـارك هـذا الكـود مع اي احـد ⚠️؟!**\n**- فقـط قم بنسخـه ولصقـه في مربعـات التنصيب 🛃**\n\n**- سـورس الإمــــبراطور : @A_S_A_A_Q**",
                )
                print(
...
def pyro_session():
    ...
    with Client(
        name="Reed", api_id=API_ID, api_hash=API_HASH, in_memory=True
    ) as pyro:
