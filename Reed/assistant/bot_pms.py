    ...
    if chat.id != Config.OWNER_ID:
        ...
        if customstrmsg is not None:
            ...
            start_msg = f"**❈╎مـرحباً بـك عزيـزي  {mention} **\
                        ...
                        \n\n**❈╎البـوت خـاص بسـورس :** [الإمبراطور 𓅛](https://t.me/A_S_A_S_W)"
        buttons = [
            (
                Button.url("قنـاة السـورس", "https://t.me/A_S_A_S_W"),
                Button.url(
                    "مطـور السـورس",
                    "https://t.me/A_S_A_A_Q",
                ),
            )
        ]
    else:
...
@zedob.bot_cmd(incoming=True, func=lambda e: e.is_private)
...
@zedob.bot_cmd(pattern="^/uinfo$", from_users=Config.OWNER_ID)
async def bot_start(event):
    reply_to = await reply_id(event)
    if not reply_to:
    ...
    if found:
        if flood_count >= FloodConfig.AUTOBAN:
            if user_.id in Config.SUDO_USERS:
                ...
                if BOTLOG:
                    await zedob.tgbot.send_message(BOTLOG_CHATID, sudo_spam)
            else:
        ...
        if not fa_id:
            return
        try:
            msg_ = await zedob.tgbot.get_messages(BOTLOG_CHATID, fa_id)
        ...
        if BOTLOG:
            fa_msg = await zedob.tgbot.send_message(
                BOTLOG_CHATID,
                flood_msg,
                buttons=buttons,
            )
        try:
            chat = await zedob.tgbot.get_entity(BOTLOG_CHATID)
            await zedob.tgbot.send_message(
                Config.OWNER_ID,
            ...
            if BOTLOG:
                await zedob.tgbot.send_message(BOTLOG_CHATID, "**- قم بالغـاء حظـر بوتك المسـاعـد ؟!**")
    if FloodConfig.ALERT[user_.id].get("fa_id") is None and fa_msg:
        FloodConfig.ALERT[user_.id]["fa_id"] = fa_msg.id


@zedob.tgbot.on(CallbackQuery(data=re.compile(b"bot_pm_ban_([0-9]+)")))
@check_owner
async def bot_pm_ban_cb(c_q: CallbackQuery):
    user_id = int(c_q.pattern_match.group(1))
    try:
        user = await zedob.get_entity(user_id)
    except Exception as e:
...
@zedob.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
...
@zedob.bot_cmd(incoming=True, func=lambda e: e.is_private)
@zedob.bot_cmd(edited=True, func=lambda e: e.is_private)
