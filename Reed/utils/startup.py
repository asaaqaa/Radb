...
# This file is a part of < https://github.com/asaaaa/Radb/ >
# PLease read the GNU Affero General Public License in
# <https://www.github.com/asaaqaa/Radb/blob/main/LICENSE/>.
...
from Reed import BOTLOG, BOTLOG_CHATID, PM_LOGGER_GROUP_ID

from ..Config import Config
from ..core.logger import logging
from ..core.session import zedob
...
async def setup_bot():
    """
    To set up bot for A_S_A_S_W
    """
    try:
        await zedob.connect()
        config = await zedob(functions.help.GetConfigRequest())
        for option in config.dc_options:
            if option.ip_address == zedob.session.server_address:
                if zedob.session.dc_id != option.id:
                    LOGS.warning(
                        f"ايـدي DC ثـابت فـي الجلسـة مـن {zedob.session.dc_id}"
                        f" الـى {option.id}"
                    )
                zedob.session.set_dc(option.id, option.ip_address, option.port)
                zedob.session.save()
                break
        bot_details = await zedob.tgbot.get_me()
        Config.TG_BOT_USERNAME = f"@{bot_details.username}"
        # await zedob.start(bot_token=Config.TG_BOT_USERNAME)
        zedob.me = await zedob.get_me()
        zedob.uid = zedob.tgbot.uid = utils.get_peer_id(zedob.me)
        if Config.OWNER_ID == 0:
            Config.OWNER_ID = utils.get_peer_id(zedob.me)
...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.zedobLOGO = await zedob.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/f821d27af168206b472ad.mp4",
                caption="**•⎆┊تـم بـدء تشغـيل سـورس الإمــــبراطور الخاص بك .. بنجاح 🧸♥️**",
                buttons=[(Button.url("الإمــــبراطور𓅛", "https://t.me/A_S_A_S_W"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
    ...
    try:
        if msg_details:
            await zedob.check_testcases()
            message = await zedob.get_messages(msg_details[0], ids=msg_details[1])
            text = message.text + "\n\n**•⎆┊تـم اعـادة تشغيـل السـورس بنجــاح 🧸♥️**"
            await zedob.edit_message(msg_details[0], msg_details[1], text)
...
async def mybot():
    ...
    zilbot = await zedob.tgbot.get_me()
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            ...
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", "هنا زلزال اليسع ")
...
Reed = {"@kapo00s", "@kapo00s", "@kapo00s", "@A_S_A_S_W"}
async def saves():
   for Cat in Reed:
        try:
             await zedob(JoinChannelRequest(channel=Cat))
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        path = f"Reed/{folder}/*.py"
        plugin_path = f"Reed/{folder}"
    ...
    if extfolder:
        if not failure:
            failure.append("None")
        await zedob.tgbot.send_message(
            BOTLOG_CHATID,
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        try:
            entity = await zedob.get_entity(BOTLOG_CHATID)
        ...
        photozed = await zedob.upload_file(file="zedthon/malath/Zpic.jpg")
        _, groupid = await create_supergroup(
            "كـروب السجـل زدثـــون", zedob, Config.TG_BOT_USERNAME, descript, photozed
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        try:
            entity = await zedob.get_entity(PM_LOGGER_GROUP_ID)
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
                    LOGS.info(
        ...
        photozed = await zedob.upload_file(file="zedthon/malath/Apic.jpg")
        _, groupid = await create_supergroup(
            "كـروب التخـزين", zedob, Config.TG_BOT_USERNAME, descript, photozed
        )
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    if response.code != 200:
        LOGS.error(errtext)
        return await zedob.tgbot.send_message(BOTLOG_CHATID, errtext)
    await runcmd(gcmd)
    if not os.path.exists(cfolder):
        LOGS.error(
            "- حدث خطأ اثناء استدعاء رابط الملفات الاضافية .. قم بالتأكد من الرابط اولاً..."
        )
        return await zedob.tgbot.send_message(
            BOTLOG_CHATID,
            "**- حدث خطأ اثناء استدعاء رابط الملفات الاضافية .. قم بالتأكد من الرابط اولاً...**",
        )
    if os.path.exists(rpath):
        await runcmd(f"pip3 install --no-cache-dir -r {rpath}")
    await load_plugins(folder="Reed", extfolder=cfolder)
