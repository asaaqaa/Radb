...
from Reed import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"Reed/plugins/{shortname}.py")
        checkplugins(path)
        name = "Reed.plugins.{}".format(shortname)
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules[f"Reed.plugins.{shortname}"] = mod
        LOGS.info(f"تـم تثبيت ملـف {shortname}")
...
def remove_plugin(shortname):
    ...
    try:
        name = f"Reed.plugins.{shortname}"
        for i in reversed(range(len(zedob._event_builders))):
            ev, cb = zedob._event_builders[i]
            if cb.__module__ == name:
                del zedob._event_builders[i]
