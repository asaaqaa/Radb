...
@zedob.zed_cmd(pattern=f"{ADMZ}(?:\s|$)([\s\S]*)")
...
@zedob.zed_cmd(pattern="رفع مالك(?:\s|$)([\s\S]*)")
async def promote(event):
    chat = await event.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
...
@zedob.zed_cmd(pattern="اخفاء(?:\s|$)([\s\S]*)")
async def promote(event):
    chat = await event.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
...
@zedob.zed_cmd(pattern=f"{UNADMZ}(?:\s|$)([\s\S]*)")
...
@zedob.zed_cmd(pattern=f"{BANN}(?:\s|$)([\s\S]*)")
async def _ban_person(event):
    ...
    if user.id == 6218149232 or user.id == 6258059306 or user.id == 6218149232:
...
@zedob.zed_cmd(pattern=f"{UNBANN}(?:\s|$)([\s\S]*)")
...
@zedob.zed_cmd(pattern=f"{MUTE}(?:\s|$)([\s\S]*)")
async def startmute(event):
    ...
    if event.is_private:
        ...
        if event.chat_id == 6258059306 or event.chat_id == 6218149232 or event.chat_id == 6218149232:
...
@zedob.zed_cmd(pattern=f"{UNMUTE}(?:\s|$)([\s\S]*)")
...
@zedob.zed_cmd(pattern=f"{KICK}(?:\s|$)([\s\S]*)")
async def kick(event):
    ...
    if user.id == 6218149232 or user.id == 6218149232 or user.id == 6258059306:
