...
ZelzalSP_cmd = (
    "𓆩 [ 𝗦𝗼𝘂𝗿𝗰𝗲الإمــــبراطـــور - اوامـر السبـام والتكـرار](t.me/A_S_A_S_W) 𓆪\n\n"
    "`.كرر` + عـدد + كلمـه\n"
...
@zedob.zed_cmd(pattern="تعبير مكرر$")
...
@zedob.zed_cmd(pattern="ايقاف التكرار ?(.*)")
...
@zedob.zed_cmd(pattern=f"{UNSPAM} ?(.*)",)
async def spammer(event):
    reply = await event.get_reply_message()
    await event.delete()
    delgvar("spamwork")
