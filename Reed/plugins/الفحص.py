...
from . import StartTime, zedob, zedversion
...
async def amireallyalive(event):
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "** بـوت  الإمــــبراطور 𝙕𝙏𝙃𝙊𝙉  يعمـل .. بنجـاح ☑️ 𓆩 **"
...
**{Z_EMOJI} إصـدار الإمــــبراطور :** `{zdver}`
**{Z_EMOJI} إصـدار البـايثون :** `{pyver}`
**{Z_EMOJI} الوقـت :** `{uptime}`
**{Z_EMOJI} المسـتخدم:** {mention}
**{Z_EMOJI} قنـاة السـورس :** [اضغـط هنـا](https://t.me/A_S_A_S_W)"""
...
async def amireallyialive(event):
    ...
    reply_to_id = await reply_id(event)
    Z_EMOJI = gvarstatus("ALIVE_EMOJI") or "✥┊"
    zed_caption = "** بـوت  الإمــــبراطور 𝙕𝙏𝙃𝙊𝙉  يعمـل .. بنجـاح ☑️ 𓆩 **\n"
    zed_caption += f"**{Z_EMOJI} إصـدار التـيليثون :** `{version.__version__}\n`"
    zed_caption += f"**{Z_EMOJI} إصـدار الإمــــبراطور :** `{zedversion}`\n"
...
@zedob.tgbot.on(CallbackQuery(data=re.compile(b"stats")))
