...
from . import ALIVE_NAME, BOTLOG, BOTLOG_CHATID, zedob, edit_delete, get_user_from_event
...
@zedob.zed_cmd(pattern="انتحال(?:\s|$)([\s\S]*)")
async def _(event):
...
@zedob.zed_cmd(pattern=f"{ANTHAL}$")
async def revert(event):
    firstname = gvarstatus("FIRST_NAME") or ALIVE_NAME
    lastname = gvarstatus("LAST_NAME") or ""
    bio = gvarstatus("DEFAULT_BIO") or "{وَتَوَكَّلْ عَلَى ٱلْحَىِّ ٱلَّذِى لَا يَمُوتُ.....}"
    await event.client(
