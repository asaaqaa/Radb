...
ZelzalPRO_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 الإمــــبراطور𓅛 𝗖𝗼𝗻𝗳𝗶𝗴 - اوامـر الاذا؏ـــة](t.me/A_S_A_S_W) 𓆪\n\n"
    ...
    "\n 𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 الإمــــبراطور𓅛](t.me/A_S_A_S_W) 𓆪"
)


# Copyright (C) 2022 Zed-Thon . All Rights Reserved
@zedob.zed_cmd(pattern="الاذاعه")
async def cmd(zelzallll):
    await edit_or_reply(zelzallll, ZelzalPRO_cmd)


@zedob.zed_cmd(pattern=f"للكروبات(?: |$)(.*)")
...
@zedob.zed_cmd(pattern=f"للمجموعات(?: |$)(.*)")
async def gcast(event):
    zedthon = event.pattern_match.group(1)
    if zedthon: #Write Code By T.me/zzzzl1l
        await edit_or_reply(event, "**⎉╎بالـࢪد ؏ــلى ࢪسـالة او وسائـط**")
...
async def pmto(event):
    ...
    if zelzal == "":
        return
    try:
        await zedob.send_message(chat_id, zelzal)
