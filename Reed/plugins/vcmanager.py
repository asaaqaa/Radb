...
async def get_group_call(chat):
    if isinstance(chat, Channel):
        result = await zedob(functions.channels.GetFullChannelRequest(channel=chat))
    elif isinstance(chat, Chat):
        result = await zedob(functions.messages.GetFullChatRequest(chat_id=chat.id))
...
async def parse_entity(entity):
    if entity.isnumeric():
        entity = int(entity)
    return await zedob.get_entity(entity)
...
async def start_vc(event):
    ...
    vc_chat = await zedob.get_entity(event.chat_id)
...
async def end_vc(event):
    ...
    if not gc_call:
        return
    try:
        await zedob(functions.phone.DiscardGroupCallRequest(call=gc_call))
...
async def title_vc(event):
    ...
    await zedob(functions.phone.EditGroupCallTitleRequest(call=gc_call, title=title))
