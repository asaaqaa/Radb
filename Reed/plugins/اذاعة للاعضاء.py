...
# <https://www.github.com/asaaqaa/ASAAQALIAS/blob/master/LICENSE/>.
...
# =========================================================== #
#                           الملـــف كتـــابـــة مـــن الصفـــر - T.me/A_S_A_S_W                           #
# =========================================================== #
Warn = "تخمـط بـدون ذكـر المصـدر - ابلعــك نعــال وراح اهينــك"
ZEDTHON_BEST_SOURCE = "[ᯓ الإمــــبراطور 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة خـاص 🚹](t.me/A_S_A_S_W) .\n\n**- جـارِ الاذاعـه خـاص لـ أعضـاء الكـروب 🛗\n- الرجـاء الانتظـار .. لحظـات ⏳**"
ZEDTHON_PRO_SOURCE = "[ᯓ الإمــــبراطور 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة زاجـل 🕊](t.me/A_S_A_S_W) .\n\n**- جـارِ الاذاعـه لـ قائمـة زاجـل 📜\n- الرجـاء الانتظـار .. لحظـات ⏳**"
ZELZAL_PRO_DEV = "[ᯓ الإمــــبراطور 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة زاجـل 🕊](t.me/A_S_A_S_W) .\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n**⎉╎قائمـة الاذاعـه فارغـه ؟! ❌**\n**⎉╎قم باضافة يوزرات عبـر الامر**\n`.اضف زاجل` **بالـرد ع عدة يوزرات تفصل بينهم مسافات**"
# =========================================================== #
...
@zedob.zed_cmd(pattern=f"للكل(?: |$)(.*)", groups_only=True)
    ...
    async for usr in event.client.iter_participants(event.chat_id):
        ...
        if zilzal.text:
            ...
            try:
                ...
                return
    ZELZAL_BEST_DEV = f"[ᯓ الإمــــبراطور 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة خـاص 🚹](t.me/A_S_A_S_W) .\n\n**⎉╎تمت الاذاعـه لـ اعضـاء الكـروب .. بنجـاح  ✅**\n**⎉╎عـدد {success} عضـو**"
...
@zedob.zed_cmd(pattern="ايقاف للكل", groups_only=True)
...
async def malath(event):
    ...
    for user in users:
        ...
        if zilzal.text:
            try:
                user_entity = await zedob.get_entity(user)
                if user_entity.bot or user_entity.deleted:
                    continue
                await zedob.send_message(user_entity.id, zilzal, link_preview=False)
                success += 1
            ...
            try:
                ...
                if user_entity.bot or user_entity.deleted:
                    continue
                await zedob.send_file(user_entity.id, zilzal.media, caption=zilzal.text)
                success += 1
            except UserAdminInvalidError:
                pass
            except Exception as e:
                zzz.edit(f"خطأ في إرسال الرسالة إلى {user_entity.id}: {str(e)}")
    ZELZAL_BEST_DEV = f"[ᯓ الإمــــبراطور 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة زاجـل 🕊](t.me/A_S_A_S_W) .\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n**⎉╎تمت الاذاعـه .. بنجـاح  ✅**\n**⎉╎عـدد {success} أشخـاص**"
