...
@zedob.zed_cmd(pattern="(تقليد|ازعاج)$")
async def echo(event):
    if event.reply_to_msg_id is None:
        return await edit_or_reply(event, "**- بالـرد على الشخص الذي تريد ازعاجـه**")
    zedobevent = await edit_or_reply(event, "**- يتم تفعيل هذا الامر انتظر قليلا ..**")
    user, rank = await get_user_from_event(event, zedobevent, nogroup=True)
    if not user:
        return
    ...
    try:
        ...
        await edit_delete(zedobevent, f"**⎉╎خطـأ :**\n`{str(e)}`")
    else:
        await edit_or_reply(
            zedobevent,
...
@zedob.zed_cmd(pattern="(الغاء تقليد|ايقاف التقليد|ايقاف تقليد|الغاء الازعاج|ايقاف الازعاج|ايقاف ازعاج|الغاء ازعاج)$")
async def echo(event):
    if event.reply_to_msg_id is None:
...
@zedob.zed_cmd(pattern="حذف المقلدهم( للكل)?")
async def echo(event):
...
@zedob.zed_cmd(pattern="المقلدهم( للكل)?$")
async def echo(event):
    input_str = event.pattern_match.group(1)
    private_chats = ""
...
@zedob.zed_cmd(incoming=True, edited=False)
