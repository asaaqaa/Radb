...
ZelzalVP_cmd = (
    "𓆩 [سورس الإمــــبراطور ⓋⒾⓅ - اوامـر الفـارات](t.me/A_S_A_S_W) 𓆪\n\n"
    ...
    "\n𓆩 [𝙕𝙚𝙙 𝙑𝙖𝙧𝙨 - قنـاة الفـارات](t.me/A_S_A_S_W) 𓆪"
)


# Copyright (C) 2022 Zed-Thon . All Rights Reserved
@zedob.zed_cmd(pattern=r"اضف (.*)")
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        if variable in heroku_var:
            await zed.edit("**⎉╎تم تغييـر {} بنجـاح ☑️**\n**⎉╎المتغيـر : ↶**\n `{}` \n**⎉╎يتم الان اعـادة تشغيـل بـوت الإمــــبراطور يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
        else:
            await zed.edit("**⎉╎تم اضافـة {} بنجـاح ☑️** \n**⎉╎المضاف اليه :**\n `{}` \n**⎉╎يتم الان اعـادة تشغيـل بـوت الإمــــبراطور يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
...
# Copyright (C) 2022 Zed-Thon . All Rights Reserved
@zedob.zed_cmd(pattern="حذف(?:\s|$)([\s\S]*)")
async def variable(event):
    if Config.HEROKU_API_KEY is None:
        return await ed(
            event,
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الان اعـادة تشغيـل بـوت الإمــــبراطور يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, variable))
        ...
        if variable not in heroku_var:
        	return await zed.edit("**⎉╎عـذࢪاً عـزيـزي .. انت لـم تقـم باضـافـة فـار {} اصـلاً...**".format(input_str))
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الان اعـادة تشغيـل بـوت الإمــــبراطور يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, heroku_var[variable]))
...
# Copyright (C) 2022 Zed-Thon . All Rights Reserved
@zedob.zed_cmd(pattern="جلب(?:\s|$)([\s\S]*)")
async def custom_zed(event):
    ...
    if (input_str == "كليشة الحماية" or input_str == "كليشة الحمايه" or input_str == "كليشه الحماية" or input_str == "كليشه الحمايه"):
        ...
        if variable is None:
            ...
            await zed.edit("**⎉╎الفـار {} موجـود ☑️**\n**⎉╎قيمـة الفـار : ↶**\n `{}` \n\n**⎉╎قنـاة السـورس : @A_S_A_S_W**".format(input_str, variable))
...
# Copyright (C) 2022 Zed-Thon . All Rights Reserved
@zedob.zed_cmd(pattern="وقت(?:\s|$)([\s\S]*)")
async def variable(event):
    if Config.HEROKU_API_KEY is None:
        return await ed(
            event,
...
# Copyright (C) 2022 Zed-Thon . All Rights Reserved
@zedob.zed_cmd(pattern="اضف صورة (الحماية|الحمايه|الفحص|الوقتي|البوت|الاوامر) ?(.*)")
async def _(malatha):
    if malatha.fwd_from:
        return
    ...
    if malatha.reply_to_msg_id:
        ...
        if input_str in ["الحماية", "الحمايه"]:
            ...
            try:
                ...
                await zed.edit("**⎉╎تم تغييـر صـورة {} .. بنجـاح ☑️**\n**⎉╎المتغيـر : ↶**\n `{}` \n\n**⎉╎قنـاة السـورس : @A_S_A_S_W**".format(input_str, vinfo))
...
# Copyright (C) 2022 Zed-Thon . All Rights Reserved
@zedob.zed_cmd(pattern="اوامر الفارات")
async def cmd(zelzallll):
    await edit_or_reply(zelzallll, ZelzalVP_cmd)

@zedob.zed_cmd(pattern="التخصيص")
async def cmd(zelzallll):
    await edit_or_reply(zelzallll, ZelzalVP_cmd)