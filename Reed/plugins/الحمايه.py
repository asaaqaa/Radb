...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if custompmpermit is not None:
        ...
        USER_BOT_NO_WARN = f"""♡سورس♡الإمــــبراطور  **- الـرد التلقـائي 〽️**
**•─────────────────•**

❞ **مرحبـاً**  {mention} ❝

**⤶ قد اكـون مشغـول او غيـر موجـود حـاليـاً ؟!**
**⤶ ❨ لديـك** {warns} **مـن** {totalwarns} **تحذيـرات ❗❩**
...
@zedob.zed_cmd(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
async def on_new_private_message(event):
    if gvarstatus("pmpermit") is None:
        return
    chat = await event.get_chat()
    zel_dev = (6258059306, 6126535154)
...
@zedob.zed_cmd(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@zedob.tgbot.on(CallbackQuery(data=re.compile(rb"show_pmpermit_options")))
...
@zedob.tgbot.on(CallbackQuery(data=re.compile(rb"to_enquire_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⤶ عـذراً سيـدي ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك 🤷🏻‍♂"
        return await event.answer(text, cache_time=0, alert=True)
...
@zedob.tgbot.on(CallbackQuery(data=re.compile(rb"to_request_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⤶ عـذراً سيـدي ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك 🤷🏻‍♂"
        return await event.answer(text, cache_time=0, alert=True)
...
@zedob.tgbot.on(CallbackQuery(data=re.compile(rb"to_chat_with_my_master")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⤶ عـذراً سيـدي ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك 🤷🏻‍♂"
        return await event.answer(text, cache_time=0, alert=True)
...
@zedob.tgbot.on(CallbackQuery(data=re.compile(rb"to_spam_my_master_inbox")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⤶ عـذراً سيـدي ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك 🤷🏻‍♂"
        return await event.answer(text, cache_time=0, alert=True)
...
async def disapprove_p_m(event):
    ...
    if pmpermit_sql.is_approved(user.id):
        ...
        await edit_or_reply(
            event,
            f"**⎉╎المستخـدم**  [{user.first_name}](tg://user?id={user.id})\n**⎉╎تـم رفـضـه مـن أرسـال الـرسـائـل ❗**\n**⎉╎ الـسـبـب ❔  :** {reason}",
        )
...
@zedob.zed_cmd(pattern="بلوك(?:\s|$)([\s\S]*)")
...
@zedob.zed_cmd(pattern="الغاء بلوك(?:\s|$)([\s\S]*)")
...
@zedob.zed_cmd(pattern="المقبولين$")
