...
from Reed.utils import admin_cmd
...
@zedob.zed_cmd(pattern="انضمام ([\s\S]*)")
async def lol(event):
    a = event.text
    bol = a[5:]
    ...
    try:
        await zedob(functions.channels.JoinChannelRequest(bol))
        await event.edit("**- تم الانضمام بنجاح  ✓**")
    except Exception as e:
        await event.edit(str(e))


@zedob.zed_cmd(pattern="اضافه ([\s\S]*)")
async def _(event):
...
@zedob.on(admin_cmd(pattern=r"ضيف ?(.*)"))
async def get_users(event):   
    ...
    Reed = await get_chatinfo(event) ; chat = await event.get_chat()
    if event.is_private:
              return await zedb.edit("**╮  لا استطـيع اضافـة الاعضـاء هـنا 𓅫╰**")
    s = 0 ; f = 0 ; error = 'None'   
  
    await zedb.edit("**⎉╎حـالة الأضافة:**\n\n**⎉╎تتـم جـمع معـلومات الـمستخدمين 🔄 ...⏣**")
    async for user in event.client.iter_participants(Reed.full_chat.id):
                try:
