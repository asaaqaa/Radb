...
# <https://www.github.com/Zed-Thon/ZelZal/blob/Reed/LICENSE/>.
...
zilzal = zedob.uid
zed_dev = (6218149232, 6126535154, 6258059306)
...
@zedob.zed_cmd(pattern="(ضع اشتراك الخاص|وضع اشتراك الخاص)(?:\s|$)([\s\S]*)")
async def _(event):
...
@zedob.zed_cmd(pattern="(تفعيل اشتراك الخاص|تفعيل الاشتراك خاص)")
...
@zedob.zed_cmd(pattern="(تعطيل اشتراك الخاص|تعطيل الاشتراك الخاص)")
...
async def f(event):
    ...
    if zelzaal:
        try:
            ...
            if rip is False:
                c = await zedob.get_entity(ch)
                chn = c.username
                if c.username == None:
                    ra = await zedob(ExportChatInviteRequest(ch))
                    chn = ra.link
                if chn.startswith("https://"):
                    await event.reply(f"ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 الإمــــبراطور𓅛 - **الاشتࢪاك الإجباࢪي**\n⋆┄─┄┄─┄┄┄─┄─┄┄⋆\n\n⌔╎**مࢪحبـاً عـزيـزي 🫂** [{user.first_name}](tg://user?id={user.id}) \n⌔╎**لـ الغـاء كتمـك 🔊**\n⌔╎**يُࢪجـى الإشتـࢪاك بالقنـاة {chn} **", link_preview=False
                    )
