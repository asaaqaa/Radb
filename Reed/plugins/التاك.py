...
async def tagall(event):
  ...
  if mode == "by_cmd":
    ...
    usrnum = 0
    usrtxt = ""
    async for usr in zedob.iter_participants(event.chat_id):
      usrnum += 1
      ...
      if event.chat_id not in moment_worker:
        ...
        return
      if usrnum == 5:
        await zedob.send_message(event.chat_id, f"{usrtxt}\n\n- {msg}")
  ...
  if mode == "by_reply":
    ...
    async for usr in zedob.iter_participants(event.chat_id):
      ...
      if event.chat_id not in moment_worker:
        ...
        return
      if usrnum == 5:
        await zedob.send_message(event.chat_id, usrtxt, reply_to=msg)
        await asyncio.sleep(2)
        usrnum = 0
        usrtxt = ""
...
@zedob.zed_cmd(pattern="ايقاف التاك?(.*)")
...
@zedob.zed_cmd(pattern="تاك(?:\s|$)([\s\S]*)")
async def tagall(event):
  global moment_worker
  if event.is_private:
    return await edit_or_reply(event, "**- عـذراً ... هـذه ليـست مجمـوعـة ؟!**")
...
@zedob.zed_cmd(pattern="منشن ([\s\S]*)")
async def _(event):
